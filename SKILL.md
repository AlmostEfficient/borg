---
name: borg
description: Bootstrap the borg PM operating system into the current project — a folder-based task queue, PM protocol, product context, and workbook for coordinating coding agents. Use this skill whenever the user says "load up borg", "init borg", "start borg", "set up borg", "wire up borg", or describes wanting to set up a PM agent workflow, task queue, borg/ folder, or structured way for spawning subagents on a project. Also trigger when the user names work to do AND mentions borg, PM agent, or task queue in the same breath, even if they don't say "init" — that means they want bootstrap plus initial task seeding in one go.
---
...
do not trigger on:
- a generic mention of tasks or PM without a borg reference
...
look for `borg/` in cwd.

- if absent → proceed to scaffold.
- if present → don't overwrite. ask the user whether they want to (a) add new tasks to the existing queue, (b) re-read the protocol, or (c) reset (destructive — requires running scaffold with both `--force --yes-really-delete-borg`). confirm with the user twice before resetting.

knowing why matters: borg's whole premise is that the `borg/` folder accumulates project memory (workbook entries, rejected approaches, work logs). silently overwriting it erases the thing that makes borg useful past day one.

### 2. scaffold

run the scaffold script. it copies `assets/borg/` into `<cwd>/borg/` and refuses to overwrite an existing `borg/` without both destructive flags.

```sh
<path-to-skill>/scripts/scaffold.sh
# or, if borg is installed on PATH:
borg
```
...
ask the user directly to fill `PRODUCT-CONTEXT.md`. if the harness exposes a structured-question tool (e.g. `AskUserQuestion` in Claude Code, `request_user_input` in Codex when available), use it; otherwise just ask in chat. ask only what isn't already obvious from the conversation. the goal is to give the future PM agent enough project-specific context that it doesn't need to re-derive it from chat history.

minimum fields:
- **what is this project, in one sentence**
- **primary verification command** (e.g. `bun test`, `cargo test`, `xcodebuild ... build`) — accept "none yet" but do not leave blank
...
borg's PM agent is both taskmaster and orchestrator (see `assets/borg/PM-PROTOCOL.md` → scoping). the bootstrap only seeds tasks when the user already named pre-scoped items.

- **named pre-scoped work** ("we're gonna do x, y, z" where x/y/z are concrete tasks): create one file per item in `borg/tasks/ready/`. use `borg/templates/task.md`. number sequentially: `001-<slug>.md`. fill in `Goal` from what the user said. leave `Acceptance checks`, `Files / areas`, and `Work log` as prompts for the PM agent.
...
> you are the PM agent for this project. read `borg/PM-PROTOCOL.md`, `borg/PRODUCT-CONTEXT.md`, `borg/WORKBOOK.md`, then list `borg/tasks/ready/` and `borg/tasks/doing/`. pick the next task and propose how you'll execute it. do not implement code yourself — delegate to subagents per the protocol.

**if the queue is empty (end-state goal case):**

> you are the PM agent for this project. read `borg/PM-PROTOCOL.md`, `borg/PRODUCT-CONTEXT.md`, `borg/WORKBOOK.md`. i'm going to brief you on what we're trying to ship. once we agree on scope, write the task files into `borg/tasks/ready/` following the protocol's scoping section, then begin execution. if our scoping conversation gets long, tell me and we'll reset you to a fresh session — the task files are the contract.

mention that new tasks go in `borg/tasks/ready/` and status changes by `mv`-ing files between status folders.
...
**assistant actions:**
1. scaffolds `borg/` via the script
...
**user:** "set up borg" (in a project where `borg/` already exists)

**assistant actions:**
1. detects existing `borg/`
